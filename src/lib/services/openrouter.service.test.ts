...
describe("OpenRouterService", () => {
  ...
  describe("Constructor", () => {
    ...
    it("should use custom baseUrl when provided", async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith("https://custom-api.example.com/v1/chat/completions", expect.any(Object));
    });
  ...
  describe("Response Validation (validateResponse)", () => {
    ...
    it("should throw error when choices is missing", async () => {
      ...
      await expect(service.generate({ userMessage: "test" })).rejects.toThrow(
        /invalid response shape \(missing choices\)/
      );
    });
    ...
    it("should throw error when message is missing", async () => {
      ...
      await expect(service.generate({ userMessage: "test" })).rejects.toThrow(
        /invalid response shape \(missing content\)/
      );
    });
